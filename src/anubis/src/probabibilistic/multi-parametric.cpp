...
namespace jmaerte {
    namespace anubis {
        namespace probabilistic {
            ...
            void proceed(mp_experiment* exp, mp_experiment::node * node, potence * potence) {
                while (!potence.done()) {
                    int[] simplex = new int[potence->order()];
                    for (int i = 0; i < potence->order(); i++) simplex[i] = potence->get(i);

                    if (node->complex->is_external(simplex)) {
                        mp_experiment::node * next = new mp_experiment::node;
                        node->children.insert(next);
                        next->complex = node->complex->im_insert(simplex);
                        next->parent = node;
                        next->prob = exp->p(potence->order() - 1) * node->prob;
                        next->is_P = exp->P(next->complex);

                        potence* pot = potence->copy();
                        delete simplex;

                        proceed(exp, next, pot);
                    } else {
                        node->prob *= exp->p(potence->order() - 1)
                        (*potence)++;
                        delete simplex;
                    }
                }
            ...
            void mp_experiment::generate_poset() {
                ...
                proceed(this, root, new potence(vertices, 1));
            }
