 ...
 **********************************************************************************************************************/
                ...
                void ADD(ap_int a, ULL lambda, int shift, ap_int const b) {
                    if (GET_SIGN(a) != GET_SIGN(b)) {
                        ...
                        if (GET_OCC(a) <= GET_OCC(b) + shift) {
                            n = GET_OCC(b) + shift;
                            if ((lambda * GET_LEADING(b)) & H_MASK) n++;
                        } else {
                            n = GET_OCC(a);
                        }
                        ENLARGE(a, n);
                        ...
                        for (int i = 0; i < n; i++) {
                            val = GET(a, i);
                            carry_lambda += lambda * GET(b, i - shift);
                            SET(a, i, val + (carry_lambda & L_MASK) + d);
                ...
                void SUB(ap_int a, ULL lambda, int shift, ap_int const b) {
                    if (GET_SIGN(a) != GET_SIGN(b)) {
                        ...
                        std::cout << n << std::endl;
                        for (; n > 0 && GET(a, n - 1) == ((lambda * GET(b, n - 1 - shift)) & L_MASK) + ((lambda * GET(b, n - 2 - shift)) >> 32); ) {
                            std::cout << GET(a, n - 1) << std::endl;
                            REMOVE(a, --n);
                        }
                        std::cout << n << std::endl;
                        sign = GET(a, n - 1) > ((lambda * GET(b, n - 1 - shift)) & L_MASK) + ((lambda * GET(b, n - 2 - shift)) >> 32) == GET_SIGN(a);
                        ENLARGE(a, n);
                        SET_OCC(a, n);
                        ULL val = 0ULL;
                        ULL carry_lambda = 0ULL;
