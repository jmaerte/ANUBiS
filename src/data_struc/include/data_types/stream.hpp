...
#ifndef ANUBIS_DATA_STRUC_SUPERBUILD_STREAM_HPP
#define ANUBIS_DATA_STRUC_SUPERBUILD_STREAM_HPP
...
namespace jmaerte {
    namespace typings {
        ...
        static stream<int> ints_from(int n) {
            return stream<int>([n]() {
        ...
        static void forEach(stream<T> strm, F f) {
            while (!strm.is_empty()) {
                f(strm.get());
                strm = strm.pop_front();
            }
        ...
        template<class T, class F>
        static auto transform(stream<T> strm, F f) -> stream<decltype(f(strm.get()))> {
        ...
        static auto zip(const stream<T> & s1, const stream<S> & s2, F f) -> stream<decltype(f(s1.get(), s2.get()))> {
...
#endif // ANUBIS_DATA_STRUC_SUPERBUILD_STREAM_HPP